...
#facerec_elf = "/usr/share/vitis_ai_library/models/facerec_resnet20/facerec_resnet20.elf"
facerec_elf = "/usr/share/vitis_ai_library/models/facerec_resnet64/facerec_resnet64.elf"
facerec_graph = xir.graph.Graph.deserialize(pathlib.Path(facerec_elf))
facerec_subgraphs = get_subgraph(facerec_graph)
assert len(facerec_subgraphs) == 1 # only one DPU kernel
facerec_dpu = runner.Runner(facerec_subgraphs[0],"run")
