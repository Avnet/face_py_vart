...
class CentroidTracker():
	...
	def update(self, rects):
		...
		#
		inputBBoxes = np.zeros((len(rects),4), dtype="int")
		#
		...
		for (i, (startX, startY, endX, endY)) in enumerate(rects):
			...
			#
			# debug
			#print(startX,startY,endX,endY,cX,cY);
		...
		# centroids
			...
			existingCentroids = np.array(objectCentroids)
			N1 = existingCentroids.shape[0]
			N2 = inputCentroids.shape[0]
			D = np.zeros( (N1,N2) )
			for i1 in range(0,N1):
				x1 = existingCentroids[i1,0]
				y1 = existingCentroids[i1,1]
				for i2 in range(0,N2):
					x2 = inputCentroids[i2,0]
					y2 = inputCentroids[i2,1]
